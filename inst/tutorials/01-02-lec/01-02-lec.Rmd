...
p_prior_mu <- ggplot(data = data.frame(x = c(-5, 5)), aes(x)) +
...
p_prior_s2 <- ggplot(data = data.frame(x = c(0, 0.2)), aes(x)) +
  stat_function(
    fun = function(x, shape, rate)
      1 / dgamma(x = x, shape = shape, rate = rate),
    n = 10001,
    args = list(shape = 20, rate = 200),
    geom = "polygon",
    color = "black",
    fill = "red",
    alpha = 0.25
...
# Plot
posterior_data <-
  tibble::as_tibble(draws_posterior) %>%
  dplyr::mutate(i = dplyr::row_number())

# Plot prior
mu_range <- range(posterior_data$mu)
sigma_range <- range(posterior_data$tau)
